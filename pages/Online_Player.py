"""Online ladder player profile (Streamlit multipage)."""
...
from kool_elo.config import DEFAULT_DB_PATH, PROVISIONAL_GAMES_FULL_THRESHOLD
...
st.set_page_config(page_title="Kick Off 2 ELO — Online player", layout="wide")
...
db_str = st.session_state.get("_kool_online_db_path") or str(DEFAULT_DB_PATH.resolve())
db_path = Path(db_str).expanduser().resolve()
if not db_path.is_file():
    st.error(f"Online SQLite not found (`{db_path}`). Open the Home page first or fix the database path.")
    st.stop()
...
if row is None:
    st.error(f"No online player with id `{player_id}`.")
    st.stop()

title_name = str(row["display_name"])
st.title(title_name)
st.caption(f"Online ladder · `{player_id}`")
